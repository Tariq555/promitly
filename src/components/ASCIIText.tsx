...
class AsciiFilter {
  ...
  constructor(
    renderer: THREE.WebGLRenderer,
    { fontSize = 12, fontFamily = "'Courier New', monospace", charset = " .'`^\",:;Il!i~+_-?][}{1)(|/tfjrxnuvczXYUJCLQ0OZmwqpdbkhao*#MW&8%B@$", invert = true } = {}
  ) {
...
class CanvAscii {
  ...
  constructor(opts: CanvAsciiOptions, container: HTMLElement, width: number, height: number) {
    ...
    this.scene.traverse((obj) => {
      ...
      if (mesh.isMesh) {
        ...
        Object.keys(mat).forEach((k) => {
          const v = mat[k];
          if (v && typeof v === "object" && typeof (v as { dispose?: () => void }).dispose === "function") {
...
export default function ASCIIText({
  ...
  useEffect(() => {
    ...
    const create = async (el: HTMLElement, w: number, h: number) => {
      const inst = new CanvAscii({ text, asciiFontSize, textFontSize, textColor, planeBaseHeight, enableWaves }, el, w, h);
      await inst.init();
      return inst;
    };
  ...
  return (
    <div ref={containerRef} style={{ position: "absolute", width: "100%", height: "100%" }}>
      <style>{`
        ...
        .ascii-canvas { position:absolute;left:0;top:0;width:100%;height:100%;image-rendering:pixelated; }
        .ascii-pre {
          margin:0;user-select:none;padding:0;line-height:1em;text-align:left;
          position:absolute;left:0;top:0;
          background-image: radial-gradient(circle, #00e5c8 0%, #00a8ff 50%, #00ff88 100%);
